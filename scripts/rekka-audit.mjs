...
const riskyItems = itemComponents.filter((row) => row.semanticStatus !== 'verified');
const riskySkills = skillComponents.filter((row) => row.semanticStatus !== 'verified');
...
${table(riskyItems, [
  { key: 'nid', label: 'NID' },
  { key: 'uses', label: 'Uses' },
  { key: 'semanticStatus', label: 'Status' },
])}
...
console.log(`- Unverified item/skill NIDs: ${riskyItems.length}/${riskySkills.length}`);
