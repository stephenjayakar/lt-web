...
function directConsumptionLocations(nid) {
  // Direct component-value reads are implementation evidence even when no
  // exported hook function exists (e.g. item.getComponent('wexp')).
  const escaped = nid.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const expression = new RegExp(
    String.raw`(?:getComponent(?:<[^>\n]*>)?|hasComponent)\(\s*['"]${escaped}['"]`,
    'g',
  );
  const locations = [];
  for (const file of tsFiles) {
...
function buildComponentManifest(kind, relativePath, webSystemPath) {
  ...
  const rows = components.map((component) => {
    ...
    const directReads = directConsumptionLocations(component.nid);
    ...
    else if (directReads.length > 0) status = 'consumed';
  ...
  const componentSummary = {
    ...
    consumedComponentNids: rows.filter((row) => row.directReads.length > 0).length,
...
function componentManifestMarkdown(manifest) {
  ...
  const rows = manifest.rows.map((component) => {
    ...
    return [
      ...
      String(component.directReads.length || ''),
      references.join('<br>'),
  ...
  return [
    ...
    '',
    `Summary: ${manifest.summary.referencedComponentNids}/${manifest.summary.pythonComponentNids} referenced in web source; ${manifest.summary.hookMappedComponentNids}/${manifest.summary.pythonComponentNids} expose at least one matching web hook; ${manifest.summary.consumedComponentNids}/${manifest.summary.pythonComponentNids} have direct getComponent/hasComponent reads.`,
    '',
...
console.log(`- Direct component reads (getComponent/hasComponent): items ${itemComponents.summary.consumedComponentNids}/${itemComponents.summary.pythonComponentNids}, skills ${skillComponents.summary.consumedComponentNids}/${skillComponents.summary.pythonComponentNids}`);
console.log('');
