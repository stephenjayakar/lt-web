...
- [ ] Preserve the remaining deep-comprehension, game-proxy-helper, and
  absent-record fallback expressions exposed by the hydrated 2,339-event
  catalog.
- [x] Preserve Python string constants, top-level list addition, and membership
  inside list comprehensions, and initialize direct-level compatibility runs
  through EotF's authored `Records_Setup` campaign prerequisites.
...
- [x] Execute both authored raw-PYEV1 events: God's Image now supports exact
...
- [x] Preserve all 40 EotF unit-selection, alliance, rescue-penalty, locktouch,
  and no-trade markers with condition/charge gates across map and base flows.
